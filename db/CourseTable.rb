...
class PostgresDirect
  ...
  def createUserTable(tableName)
    ...
    @conn.exec("CREATE TABLE #{tableName} (id SERIAL PRIMARY KEY NOT NULL, course_id character varying(255) NOT NULL, name character varying(255), slug character varying(255), course_site character varying(255), instructors character varying(255000), partners character varying(255000), homepage character varying(255000)) WITH (OIDS=FALSE);");
  end
  ...
  def addUser(course_id, name, slug, course_site, instructors, partners, homepage)
    @conn.exec_prepared("insert_user", [course_id, name, slug, course_site, instructors, partners, homepage])
  end
