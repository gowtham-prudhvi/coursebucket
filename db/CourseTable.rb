...
class PostgresDirect
  ...
  def prepareInsertUserStatement(tableName)
    @conn.prepare("insert_user", "insert into #{tableName} (course_id, name, slug, course_site, instructors) values ($1, $2, $3, $4, $5)")
  end

  # Add a user with the prepared statement.
  def addUser(course_id, name, slug, course_site, instructors)
    @conn.exec_prepared("insert_user", [course_id, name, slug, course_site, instructors])
  end
